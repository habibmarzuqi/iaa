...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    // Allow any type for archives (more permissive than media) but warn on unknown
    const mimeType = file.type || 'application/octet-stream'
    if (ALLOWED_TYPES.length > 0 && !ALLOWED_TYPES.includes(mimeType) && mimeType === 'application/octet-stream') {
      // Allow unknown types but log
      console.log(`Archive upload unknown type: ${file.name} (${mimeType})`)
    }
    ...
    await db.auditLog.create({
      data: { userId: user.id, action: 'ARCHIVE_FILE_UPLOAD', description: `Uploaded archive file ${file.name} (${(file.size / 1024).toFixed(1)} KB)` },
    })
