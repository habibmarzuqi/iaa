/**
 * Generic image upload for CMS content (articles, events, library)
 * POST /api/articles-admin/upload — multipart with file
 * Used for: article featuredImage, event coverImage, library coverImage
 */
...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const url = `/uploads/cms/${baseName}${ext}`

    let thumbUrl: string | null = null
    if (file.type !== 'image/svg+xml') {
      try {
        await sharp(buffer).resize(400, 300, { fit: 'cover' }).webp({ quality: 80 }).toFile(path.join(UPLOAD_DIR, `${baseName}-thumb.webp`))
        thumbUrl = `/uploads/cms/${baseName}-thumb.webp`
      } catch {}
    }
